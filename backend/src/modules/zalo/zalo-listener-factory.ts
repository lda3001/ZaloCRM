...
export function attachZaloListener(ctx: ListenerContext): void {
  ...
  listener.on('message', async (message: any) => {
    try {
      ...
      if (!isGroup && message.isSelf && message.threadId && api.getUserInfo) {
        contactUid = String(message.threadId);
        const userInfo = await resolveZaloName(api, contactUid, userInfoCache);
        contactName = userInfo.zaloName;
        if (userInfo.avatar) updateContactAvatar(contactUid, userInfo.avatar);
      }
      ...
      if (result && !result.isDuplicate) {
        io?.emit('chat:message', {
          ...
          threadType: isGroup ? 'group' : 'user',
          conversationName: isGroup ? groupName || result.message.senderName : result.message.senderName,
        });
      }
    } catch (err) {
